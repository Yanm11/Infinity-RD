...
/*
Description: Hash function that recieves a key and returns the index 
of the bucket it should be inserted into.
*/
typedef size_t (*hash_func_t)(void *key);

/*
Description: Action function that recieves the data of an element
 and params and does some action on the data.
*/
...
params: key size in bytes, value size in bytes, number of buckets
 to store hashed values
...
/*
HashTableIsEmpty
Descripion: Check if the table is empty. 
(do not use size func but return if find 1 element)
params: pointer to hash table.
...
/*
HashTableForEach
Descripion: Apply hash_action_func_t func for each element in the table. 
Stops when done or when action function return something other than 0(Failure).
